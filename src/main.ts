...
function handleGestureFrame(snapshot: GestureSnapshot, timestamp: number, sampleTimestamp: number, freshSample: boolean) {
  ...
  // Real ZHEN release is never inherited from the generic buffer across READY.
  if (motion.action && motion.action !== 'FLICK') inputBuffer.push(motion.action, freshSample ? sampleTimestamp : timestamp, motion.intensity, motion.direction);
  ...
  let spellMotion = zhenFlick.update({ snapshot, motion, sampleTimestamp, freshSample,
  ...
  if (snapshot.handCount && qimen.spellSystem.activeSpell?.id !== 'ZHEN_LIGHTNING' && qimen.spellSystem.stage === 'READY' && (!motion.action || motion.action === 'HOLD')) {
  ...
  events.forEach((event) => {
    if (event.type === 'SUMMON') {
      ...
      if (sector !== null) {
        ...
        selectedSector = sector;
        zhenFlick.consumeLock(sector);
        if (sector === 6) {
          spellMotion = { ...motion, action: motion.action === 'HOLD' ? 'HOLD' : null, flickScore: 0 };
          if (zhenQaMode) zhenTrace.event(timestamp, 'LOCK_PINCH_CONSUMED', 'ZHEN');
        }
  ...
  const zhenDiagnostics = { ...zhenFlick.status };
  if (zhenFlick.status.castGate) lastActionSampleAt = spellMotion.timestamp;
  ...
  if (freshSample) recorder.record(snapshot, spellMotion, qimen.spellSystem.stage, qimen.spellSystem.lockedSector, sampleTimestamp, zhenDiagnostics);
  ...
  debug.updateQa(zhenQaMode ? zhenTrace.lines({ snapshot, candidate: stabilized.gestureCandidate, stable: stabilized.stableGesture,
    ...
    status: zhenDiagnostics, threshold: tuning.values.flickThreshold, timestamp }) : kunQaMode ? [
