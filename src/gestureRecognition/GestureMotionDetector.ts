...
export class GestureMotionDetector {
  ...
  update(snapshot: GestureSnapshot, timestamp: number): MotionState {
    if (this.lastOutput && timestamp <= this.lastOutput.timestamp) return { ...this.lastOutput, action: null };
    ...
    const velocity = previous
      ...
      : { x: 0, y: 0, z: 0 };
    const sample: MotionSample = { timestamp, position, velocity, pinch: snapshot.pinchActive, pinchGap: snapshot.normalizedPinchDistance, handScale: snapshot.handScale, facing: snapshot.palmFacingCamera };
    ...
    if (flick) {
      ...
      const windowStart = this.windowStart(timestamp);
      if (windowStart) {
        const dx = position.x - windowStart.position.x;
        const dy = position.y - windowStart.position.y;
        if (Math.abs(dx) > params.swipeThreshold && Math.abs(dx) > Math.abs(dy) * 1.15 && speed > 0.14) {
    ...
    const swipeConsistency = swipeWindow ? Math.abs(position.x - swipeWindow.position.x) / Math.max(0.001, Math.hypot(position.x - swipeWindow.position.x, position.y - swipeWindow.position.y)) : 0;
    ...
    const swipeScore = Math.min(1, (Math.abs(swipeVelocity) / Math.max(params.swipeThreshold * 5, 0.01)) * swipeConsistency);
