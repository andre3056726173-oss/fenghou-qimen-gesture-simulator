...
export class GestureMotionDetector {
  ...
  update(snapshot: GestureSnapshot, timestamp: number): MotionState {
    ...
    if (!center) {
      this.reset();
      return { action: null, velocity: { x: 0, y: 0, z: 0 }, depthVelocity: 0, swipeVelocity: 0, speed: 0, stableMs: 0, holdTime: 0, intensity: 0.75, direction: { x: 0, y: 0, z: 0 }, anticipation: 0, pushScore: 0, pullScore: 0, swipeScore: 0, flickScore: 0, swipeDirectionConsistency: 0, pushEvidence: 0, pullEvidence: 0, swipeDisplacement: 0, pinchSeparationVelocity: 0, timestamp, scaleRate: 0, pullZEvidence: 0, pullScaleEvidence: 0, pullFacingEvidence: 0, swipe: emptySwipe('HAND_LOST') };
    ...
    this.lastOutput = { action, velocity, depthVelocity: velocity.z, swipeVelocity, speed, stableMs, holdTime: stableMs, intensity, direction, anticipation, pushScore, pullScore, swipeScore, flickScore, swipeDirectionConsistency: swipeConsistency, pushEvidence: forwardEvidence, pullEvidence, swipeDisplacement: Math.abs(swipe.horizontal), pinchSeparationVelocity: pinchReleaseSpeed, timestamp, swipe, scaleRate, pullZEvidence, pullScaleEvidence, pullFacingEvidence };
